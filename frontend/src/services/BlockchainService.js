...
class BlockchainService {
    ...
    async connectWallet() {
        ...
        if (chainId !== expectedChainId) {
            throw new Error(
                "MetaMask is not connected to the correct network. Please switch to Localhost 8545 (chainId 31337) in MetaMask."
            );
        }
    ...
    async mintDegree(
        ...
        try {
            ...
            console.warn(
                "callStatic failed (normal if revert expected):",
                err.message
            );
        }
        ...
        if (!finalTokenId) {
            throw new Error(
                "mintDegree: tokenId not found in event or callStatic"
            );
        }
    ...
    async hasRole(roleName, address) {
        const { MINISTRY_ROLE, UNIVERSITY_ROLE, DEFAULT_ADMIN_ROLE } =
            await this._fetchRoles();
        let role;
    ...
    async fetchMetadataFromURI(metadataURI) {
        try {
            let url =
                `https://${import.meta.env.VITE_GATEWAY_URL}/ipfs/` +
                metadataURI;
