...
class IpfsService {
    ...
    async initializeGroups() {
        try {
            this.Degree_File = await this.ensureGroupExists(
                "CertChain_Degree_File"
            );
            this.Degree_Metadata = await this.ensureGroupExists(
                "CertChain_Degree_Metadata"
            );
        } catch (error) {
    ...
    async ensureGroupExists(groupName) {
        try {
            const result = await this.pinata.groups.public
                .list()
                .name(groupName);
